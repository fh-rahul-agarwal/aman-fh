...
class SSleepTypeAgg:
    ...
    def process(self):
        ...
        sleep_data_processor.loc[:, 'date'] = pd.to_datetime(sleep_data_processor['creationDate']).dt.date
        
        # Aggregate the durations for each sleep type
        agg_df = sleep_data_processor.groupby(
            ['userName', 'valueGeneratedAt', 'sourceName', 'date', 'unit', 'type', 'value'],
            as_index=False
        )['duration'].sum()
        ...
        total_sleep_duration = total_sleep_df.groupby(
            ['userName', 'valueGeneratedAt', 'sourceName', 'date', 'unit'], 
            as_index=False
        )['duration'].sum()
        ...
        value_type_mapping = {
            'HKCategoryValueSleepAnalysisInBed' : 'TotalInBedDuration',
            'HKCategoryValueSleepAnalysisAwake' : 'TotalAwakeDuration',
            'HKCategoryValueSleepAnalysisAsleepCore' : 'TotalCoreSleepDuration',
            'HKCategoryValueSleepAnalysisAsleepDeep' : 'TotalDeepSleepDuration',
            'HKCategoryValueSleepAnalysisAsleepREM' : 'TotalREMSleepDuration'
        }
