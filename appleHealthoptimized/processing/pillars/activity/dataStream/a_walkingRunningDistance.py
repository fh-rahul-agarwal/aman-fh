...
class AWalkingRunningDistance:
    def __init__(self, records_df, *args):
        try:
            # logger.start('AWalkingRunningDistance', '__init__', "Initiated")

            self.records_df = records_df[records_df["type"] == 'HKQuantityTypeIdentifierDistanceWalkingRunning'].copy()
            ...
            # logger.success('AWalkingRunningDistance', '__init__', "Completed")

        except Exception as e:
            logger.error('AWalkingRunningDistance', '__init__', f"Error during initialization: {str(e)}")

    def _filter_by_single_date(self, start_date):
        # logger.start('AWalkingRunningDistance', '_filter_by_single_date', "Initiated")
        try:
            result = self._filter_data(start_date)
            # logger.success('AWalkingRunningDistance', '_filter_by_single_date', "Completed")
            return result
        except Exception as e:
            logger.error('AWalkingRunningDistance', '_filter_by_single_date', f"Error during filtering by single date: {str(e)}")

    def _filter_by_date_range(self, start_date, end_date):
        # logger.start('AWalkingRunningDistance', '_filter_by_date_range', "Initiated")
        try:
            result = self._filter_data(start_date, end_date)
            # logger.success('AWalkingRunningDistance', '_filter_by_date_range', "Completed")
            return result
        except Exception as e:
            logger.error('AWalkingRunningDistance', '_filter_by_date_range', f"Error during filtering by date range: {str(e)}")

    def _filter_by_offset(self, start_date, days_offset, offset_sign):
        # logger.start('AWalkingRunningDistance', '_filter_by_offset', "Initiated")
        try:
            if offset_sign == '+':
            ...
            result = self._filter_data(start_date, end_date)
            # logger.success('AWalkingRunningDistance', '_filter_by_offset', "Completed")
            return result
        except Exception as e:
            logger.error('AWalkingRunningDistance', '_filter_by_offset', f"Error during filtering by offset: {str(e)}")

    def _filter_by_dates_list(self, dates_list):
        # logger.start('AWalkingRunningDistance', '_filter_by_dates_list', "Initiated")
        try:
            filtered_df = pd.concat([self._filter_data(date) for date in dates_list]).drop_duplicates().reset_index(drop=True)
            # logger.success('AWalkingRunningDistance', '_filter_by_dates_list', "Completed")
            return filtered_df
        except Exception as e:
            logger.error('AWalkingRunningDistance', '_filter_by_dates_list', f"Error during filtering by dates list: {str(e)}")

    def _filter_data(self, start_date, end_date=None):
        # logger.start('AWalkingRunningDistance', '_filter_data', "Initiated")
        try:
            if self.records_df.empty:
                logger.cwarning('AWalkingRunningDistance', '_filter_data', "Empty DF")
            ...
            # logger.success('AWalkingRunningDistance', '_filter_data', "Completed")
            return filtered_df.reset_index(drop=True)
        except Exception as e:
            logger.error('AWalkingRunningDistance', '_filter_data', f"Error during filtering data: {str(e)}")

    def _handle_empty_records(self):
        # logger.start('AWalkingRunningDistance', '_handle_empty_records', "Initiated")
        try:
            logger.cwarning('AWalkingRunningDistance', '_handle_empty_records', "No Flights Records Found")
            return pd.DataFrame(columns=['userName', 'valueGeneratedAt', 'type', 'sourceName', 'sourceVersion', 'creationDate', 'startDate', 
                         'endDate', 'unit', 'value'])
        except Exception as e:
            logger.error('AWalkingRunningDistance', '_handle_empty_records', f"Error during handling empty records: {str(e)}")

    def _filter_and_sum_by_source(self, df):
        # logger.start('AWalkingRunningDistance', '_filter_and_sum_by_source', "Initiated")
        try:
            if df.empty:
                logger.cwarning('AWalkingRunningDistance', '_filter_and_sum_by_source', "No Records")
                return None, None

            df['value'] = pd.to_numeric(df['value'], errors='coerce')
            grouped_df = df.groupby('sourceName')['value'].sum().reset_index()
            max_source = grouped_df.loc[grouped_df['value'].idxmax()]
            # logger.success('AWalkingRunningDistance', '_filter_and_sum_by_source', "Completed")
            return max_source['sourceName'], max_source['value']

        except Exception as e:
            logger.error('AWalkingRunningDistance', '_filter_and_sum_by_source', f"Error during filtering and summing by source: {str(e)}")

    def process(self):
        # logger.start('AWalkingRunningDistance', 'process', "Initiated")
        try:
            filtered_by_type = self.filtered_records_df

            if filtered_by_type.empty:
                logger.cwarning('AWalkingRunningDistance', 'process', "Terminated With No Flights Records Found")
                return pd.DataFrame(columns=['userName', 'valueGeneratedAt', 'type', 'sourceName', 'sourceVersion', 'creationDate', 'startDate', 
                         'endDate', 'unit', 'value'])
            ...
            if max_source_name is None:
                logger.cwarning('AWalkingRunningDistance', 'process', "No valid source found")
                return pd.DataFrame(columns=['userName', 'valueGeneratedAt', 'type', 'sourceName', 'sourceVersion', 'creationDate', 'startDate', 
                         'endDate', 'unit', 'value'])
            ...
            # logger.success('AWalkingRunningDistance', 'process', "Completed")
            return final_df[['userName', 'valueGeneratedAt', 'type', 'sourceName', 'sourceVersion', 'creationDate', 'startDate', 
                            'endDate', 'unit', 'value']].copy()
        except Exception as e:
            logger.error('AWalkingRunningDistance', 'process', f"Error during processing: {str(e)}")
